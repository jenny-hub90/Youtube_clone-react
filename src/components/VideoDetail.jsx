...
const VideoDetail = () => {
  ...
  useEffect(() => {
    ...
    .then((fetcheddata) => setVideoDetail(fetcheddata.items[0]));

    fetchFormAPI(`search?part=snippet&relatedToVideoId=${id}&type=video`)
    .then((fetcheddata) => setData(fetcheddata.items));
  },[id]);
               ...
               <Stack direction="row" justifyContent="space-between" sx={{color:'#fff'}} py={1} px={2}>
                <Link to = {`/channel/${channelId}`}>
                  <Typography variant={{sm:'subtitle1', md:'h6'}} color="#fff">
                    {channelTitle}
                    <CheckCircle sx={{fontSize:'12px', color:"red", ml:'5px'}}></CheckCircle>
                  </Typography>
